...
class schwangerView extends Ui.View {
...
class schwangerDelegate extends Ui.InputDelegate {
    function onKey(evt) {
        
        if (evt.getKey() == Ui.KEY_ENTER) {
            Sys.println("ENTER gedrückt");
            widget.cycleView();
            return true;
        }
        return false;
...
class schwangerApp extends App.AppBase {
    function onStart() {
    }
    ...
    function getInitialView() {
        widget = new schwangerView();
        return [widget, new schwangerDelegate()];
    }
}